...
| **Third-party evidence steering** | `verify` and `appeal` are restricted to the claim author, arbiter or owner via `_only_party`, so an arbitrary account cannot choose the sources that decide someone else's claim. |
| **Finalization sniping** | While a ruled claim's challenge window is open (appeal rounds remain and nobody waived), only a party may `finalize`. After it closes anyone may finalize, so liveness does not depend on the parties. |
| **Unsatisfiable configuration / permanent DoS** | `_check_corroboration_budget` rejects `min_sources > MAX_SOURCES` and `min_sources > distinct(allowed_domains)` in the constructor, `set_allowed_domains` and `set_min_sources`; allowlists are normalised and de-duplicated first, so duplicates cannot fake capacity. |

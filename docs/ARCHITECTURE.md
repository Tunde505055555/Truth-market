...
`verify`, `appeal` and `waive_challenge` call `_only_party(claim)` (author,
arbiter or owner) before any other work. `finalize` calls
`_challenge_window_closed(claim)`; if the window is still open it also requires a
party. The window is closed when `claim.challenge_closed != 0` (waived, or
deadlock reached) or `round_no >= max_rounds`. `claim.challenge_closed` is stored
per claim, so the rule is deterministic and needs no clock.

## Execution phases of `verify` / `appeal`

0. **Authorization gate** — `_only_party`, plus the challenge-window check.

...
-----------------------
Choosing the evidence *is* choosing the verdict, so `verify`, `appeal` and
`waive_challenge` are restricted to the parties of a claim: its author, the
arbiter, or the owner. A ruled claim then carries a *challenge window*: it is
open while appeal rounds remain and nobody has waived them. While it is open
only a party may `finalize`, so a stranger cannot lock in a verdict before the
author can appeal; once it closes (rounds exhausted or explicitly waived)
anybody may `finalize`, so no claim can be held hostage.

CONFIGURATION INVARIANT
-----------------------
`min_sources` must be satisfiable: <= MAX_SOURCES and <= the number of distinct
allowlisted domains. Enforced in the constructor and in every owner update
(`set_allowed_domains`, `set_min_sources`), so the allowlist can never be shrunk
into a state where every `verify` reverts.
...
class Claim:
    ...
    evidence_summary: str
    round_no: u8
    # 0 = challenge window still open, 1 = closed (waived or rounds exhausted)
    challenge_closed: u8
...
def _normalize_domains(raw_domains: list[str]) -> list[str]:
    """Normalize an allowlist and drop duplicates, preserving order."""
    out: list[str] = []
    for entry in raw_domains:
        domain = _domain_of(entry)
        if domain == "":
            raise Exception("allowlist entries must be non-empty domains")
        if domain not in out:
            out.append(domain)
    return out


def _check_corroboration_budget(min_sources: int, domain_count: int) -> None:
    """A configuration that no call can ever satisfy must be rejected up front.

    `_check_sources` demands `min_sources` urls, at most `MAX_SOURCES`, each on a
    *distinct* allowlisted domain. So `min_sources` may never exceed either the
    hard cap or the number of distinct allowlisted domains, otherwise every
    `verify`/`appeal` reverts and claims are permanently unrulable.
    """
    if min_sources < 1:
        raise Exception("min_sources must be >= 1")
    if min_sources > MAX_SOURCES:
        raise Exception("min_sources must not exceed MAX_SOURCES")
    if min_sources > domain_count:
        raise Exception("min_sources must not exceed the number of distinct allowed domains")
...
class TruthMarket(gl.Contract):
    ...
    def __init__(
        ...
        _check_corroboration_budget(min_sources, len(domains))
    ...
    def set_allowed_domains(self, allowed_domains: list[str]) -> None:
        """Owner-only. Rejected if it would leave `min_sources` unsatisfiable."""
        self._only_owner()
        if len(allowed_domains) == 0:
            raise Exception("evidence allowlist must not be empty")

        domains = _normalize_domains(allowed_domains)
        _check_corroboration_budget(int(self.min_sources), len(domains))
    ...
    @gl.public.write
    def set_min_sources(self, min_sources: int) -> None:
        """Owner-only. Rejected if the current allowlist cannot satisfy it."""
        self._only_owner()
        _check_corroboration_budget(min_sources, len(self.allowed_domains))
        self.min_sources = u8(min_sources)
    ...
    def verify(self, claim_id: int, source_urls: list[str]) -> None:
        ...
        Evidence selection is a privileged act: whoever picks the urls steers the
        verdict. Only the claim author, the arbiter, or the owner may do it.
        """
        claim = self._claim(claim_id)
        self._only_party(claim)
    ...
    def appeal(self, claim_id: int, source_urls: list[str]) -> None:
        ...
        if int(claim.challenge_closed) != 0:
    ...
    @gl.public.write
    def waive_challenge(self, claim_id: int) -> None:
        """A party closes the challenge window early, giving up further appeals."""
        claim = self._claim(claim_id)
        self._only_party(claim)
        if claim.status != CS_RULED:
    ...
    def finalize(self, claim_id: int) -> None:
        """Make the recorded verdict immutable.

        Lifecycle rule: a ruled claim carries a challenge window. It is open
        while appeal rounds remain and no party has waived them. While the
        window is open only a party to the claim (author, arbiter, owner) may
        finalize — an arbitrary account cannot snipe a verdict before the author
        can appeal. Once the window has closed (rounds exhausted or waived)
        anyone may finalize, so no claim can be held hostage.
        """
        claim = self._claim(claim_id)
        if claim.status != CS_RULED:
            raise Exception("claim is not in a rulable state")
        if not self._challenge_window_closed(claim):
            self._only_party(claim)
        claim.challenge_closed = u8(1)
    ...
    def get_claim(self, claim_id: int) -> dict:
        ...
        return {
            ...
            "challenge_window_open": (
                claim.status == CS_RULED and not self._challenge_window_closed(claim)
            ),
            "appeals_remaining": max(0, int(self.max_rounds) - int(claim.round_no)),
    ...
    def _only_party(self, claim: Claim) -> None:
        """Author, arbiter and owner are the only parties to a claim's lifecycle."""
        sender = gl.message.sender_address
        if sender != claim.author and sender != self.arbiter and sender != self.owner:
            raise Exception("only the claim author, arbiter or owner may act on this claim")

    def _challenge_window_closed(self, claim: Claim) -> bool:
        """Closed when a party waived it or no appeal round remains."""
        if int(claim.challenge_closed) != 0:
            return True
        return int(claim.round_no) >= int(self.max_rounds)
